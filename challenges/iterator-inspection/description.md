...
1. `indexed_elements<T: Clone>(items: &[T]) -> Vec<(usize, T)>` - Return elements with their indices using `enumerate()`
...
4. `group_consecutive_duplicates<T: Clone + PartialEq>(items: &[T]) -> Vec<(T, usize)>` - Group consecutive identical elements and count them using `peekable()`
...
6. `collect_with_trace<T: Clone + std::fmt::Debug>(items: &[T], trace: &mut Vec<String>) -> Vec<T>` - Collect elements while recording a trace of each element using `inspect()`
7. `sum_with_running_total(numbers: &[i32], totals: &mut Vec<i32>) -> i32` - Sum numbers while recording running totals at each step using `inspect()`
